...
### Refactored
- 수수료 비중/이메일 통계 차트 → 접속률/평균 수수료/CSO 업체 수 추이 차트로 교체
- 3열+1열 그리드 레이아웃: 좌측(KPI→월별추이→하단3열) + 우측(할일+빠른이동)
- TodoPanel에 빠른 이동 섹션 추가
- EmailSystemBar 최하단 배치
...
### Refactored
- 관리자 대시보드 상태/페칭 로직 `useAdminDashboard` 훅으로 추출 (SoC 적용)
- `page.tsx` 450줄 → 165줄 (렌더링 전용), 훅 260줄 (상태+페칭+계산)
...
### Fixed
- 마스터 조회 거래처 드롭박스 비어짐 버그 수정 — `cso-companies` API가 `users` 테이블 대신 `cso_matching` 테이블을 사용하도록 변경
...
### Fixed
- 거래처매핑 배지: 관리자 업체명이 CSO관리업체로 카운트되는 문제 수정 (관리자 업체명 필터링 추가)
- 접속업체 스냅샷: 업로드 시점부터 접속 추적 시작 (업로드 전 로그인은 해당 월 데이터 부재로 무의미)
