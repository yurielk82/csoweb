...
function validateRegisterForm(
  ...
  bizStatus: BizVerificationStatus,
): string | null {
  if (bizStatus !== 'success') return '사업자등록번호 국세청 인증이 필요합니다.';
...
function buildRegisterPayload(formData: RegisterFormData) {
  return {
...
function useBizVerification(businessNumber: string) {
  ...
  }, [businessNumber, verifyBusinessNumber]);
  ...
  return { bizVerification, retryVerification };
}
...
export function useRegister() {
  ...
  const { bizVerification, retryVerification } = useBizVerification(formData.business_number);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validationError = validateRegisterForm(formData, bizVerification.status);
    if (validationError) {
  ...
  return {
    ...
    handleAddressSearch, handleRetryVerification: retryVerification, handleSubmit,
  };
}
