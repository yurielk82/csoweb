...
export function RecipientCard({
  ...
  sections, yearMonthOptions, toggleSection, moveSection,
  recipientCount, loadingCount, sending,
}: RecipientCardProps) {
  return (
    <Card>
      <CardHeader>
...
interface YearMonthOptionsProps {
  selectedYearMonth: string;
  setSelectedYearMonth: (v: string) => void;
  includeSettlementTable: boolean;
  setIncludeSettlementTable: (v: boolean) => void;
...
function YearMonthOptions({
  selectedYearMonth, setSelectedYearMonth,
  includeSettlementTable, setIncludeSettlementTable,
  sections, yearMonthOptions, toggleSection, moveSection, sending,
}: YearMonthOptionsProps) {
  return (
    <div className="ml-6 space-y-3">
      ...
      {includeSettlementTable && (
        <SectionOrderManager sections={sections} toggleSection={toggleSection} moveSection={moveSection} sending={sending} />
      )}
    </div>
  );
}
...
function SectionOrderManager({ sections, toggleSection, moveSection, sending }: SectionOrderManagerProps) {
  return (
    <div className="rounded-lg border bg-background p-3 space-y-2">
      ...
      <div className="space-y-1">
        {sections.map((section, index) => (
          ...
          >
            <Checkbox checked={section.enabled} onCheckedChange={() => toggleSection(section.id)} disabled={sending} />
            <span className="text-xs font-medium text-muted-foreground w-4">{index + 1}.</span>
            <span className={`text-sm flex-1 ${!section.enabled ? 'text-muted-foreground line-through' : ''}`}>
              {section.label}
            </span>
            <div className="flex gap-0.5">
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveSection(index, 'up')} disabled={index === 0 || sending}>
                <ChevronUp className="h-3.5 w-3.5" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveSection(index, 'down')} disabled={index === sections.length - 1 || sending}>
...
export function EmailContentCard({ subject, setSubject, body, setBody, insertVariable, sending }: EmailContentCardProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <VariableBadges insertVariable={insertVariable} sending={sending} />
      </CardContent>
    </Card>
  );
}

function VariableBadges({ insertVariable, sending }: { insertVariable: (key: string, target: 'subject' | 'body') => void; sending: boolean }) {
  return (
    <div className="space-y-2">
