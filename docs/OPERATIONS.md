...
```
git push origin main  →  Vercel 감지  →  npm run build  →  배포 완료
```
...
기존 배포 환경으로, `netlify.toml`에 설정이 남아 있습니다. 프로덕션 트래픽은 Vercel로 이전 완료.
...
3. Vercel 대시보드에서 환경변수가 최신인지 확인 (특히 새 변수 추가 시)
...
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase 서비스 키 (RLS 우회) | Vercel + `.env.local` |
| `JWT_SECRET` | JWT 서명 키 | Vercel + `.env.local` |
| `RESEND_API_KEY` | Resend 이메일 API 키 | Vercel + `.env.local` |
| `EMAIL_FROM` | 발신자 이메일 | Vercel + `.env.local` |
| `ADMIN_EMAIL` | 관리자 알림 수신 이메일 | Vercel + `.env.local` |
| `NEXT_PUBLIC_BASE_URL` | 사이트 기본 URL | Vercel(프로덕션 URL) + `.env.local`(localhost) |
...
| 빌드 실패 (Vercel) | TypeScript 에러 또는 의존성 문제 | Vercel 대시보드 → Deployments → 빌드 로그 | 로컬 `npm run build`로 재현 후 수정 |
...
| 로그 종류 | 확인 경로 |
|-----------|-----------|
| **빌드 로그** | Vercel 대시보드 → Deployments → 해당 배포 클릭 |
| **런타임 로그** (API 에러 등) | Vercel 대시보드 → Logs → Runtime |
